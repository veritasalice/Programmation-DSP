...
Void echo(Void)
{
	...
	{
		temp = sqrt(gain_graves); // on calcule la racine carré du gain une seule fois
	...
	{	
		temp = sqrt(gain_aigus); // on calcule la racine carré du gain une seule fois
		f = (-2*temp+w0_aigus)/(w0_aigus + 2/temp);
		g = (w0_aigus - 2/temp)/(w0_aigus + 2/temp);
		h = (w0_aigus + 2*temp)/(w0 + 2/temp);
	...
	for (i = 4; i < size+4; i++)
	{
		...
		BufOut[i] = ; // calcul de la sortie du filtre pour les mediums
	}
